...
async def clear_chat_history(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Timezone adjust kiya target sorting ke liye (IST Base)
    ...
    # Pichle 1500 messages tak scan run karega jisse normal user interface par lag na ho
    for msg_id in range(current_msg_id, max(1, current_msg_id - 1500), -1):
        if msg_id == status_msg.message_id:
            continue
            
        try:
            # Message verification system run kiya date nikalne ke liye
            ...
            await msg_check.delete()  # Verification copy ko turant delete kiya
            
            # CONDITION 1: /clear1 sirf purani dates (15, 16, 17) ko target karega
            if command == "/clear1" and msg_date >= today_date:
                continue
                
            # CONDITION 2: /clear2 sirf aaj ki chal rahi current date ke messages target karega
            ...
            messages_checked += 1
            if messages_checked > 100:  # Loop protection system break target
                break
            continue

    # Deletion process execution block
    deleted_count = 0
    if to_delete:
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if uid in ADMIN_IDS and uid in admin_testing_mode:
        await update.message.reply_text("⚠️ Bhai, please niche diye gaye panel ke buttons ka istemaal karein, text type na karein!")
        return
...
def main():
    ...
    # 🔥 Handlers ko upar daala taaki koi bhi user run kare toh chal jaye
